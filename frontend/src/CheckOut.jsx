...
export default function Checkout() {
  ...
  const openRazorpayModal = async (paymentMethod = null) => {
    setLoading(true);
    try {
      ...
      const options = {
        key: "rzp_test_4rdgre6savrrmw", // Ideally from env var
        amount: response.data.amount,
        currency: response.data.currency,
        name: "Abi Hollow Bricks",
        description: `Order for ${orderData.brickQuantity} ${orderData.brickType} Bricks`,
        image:
          "https://static.vecteezy.com/system/resources/previews/020/429/653/original/abi-flat-accounting-logo-design-on-white-background-abi-creative-initials-growth-graph-letter-logo-concept-abi-business-finance-logo-design-vector.jpg",
      ...
      // Force payment method if UPI is requested
      ...
      <div className={styles.checkoutContainer}>
        ...
        {paymentSuccess ? (
          ...
          <>
            ...
            <button className={styles.proceedButton} onClick={handleCashOnDelivery} disabled={loading}>
              {loading ? "Placing Order..." : "Cash on Delivery"}
            </button>

            <button className={styles.proceedButton} onClick={() => openRazorpayModal()} disabled={loading}>
              {loading ? (
                <>
            ...
            <button className={styles.proceedButton} onClick={() => openRazorpayModal("upi")} disabled={loading}>
              {loading ? (
                <>
                  <span className={styles.spinner}></span>
                  Processing...
